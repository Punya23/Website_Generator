...
export async function generateRealEstatePlacements(
...
): Promise<{ templateId: string; textCount: number; imageCount: number; outPath: string }[]> {
  const log = options.onProgress ?? ((line: string) => console.log(line));
  const results: { templateId: string; textCount: number; imageCount: number; outPath: string }[] = [];
  ...
  for (const folder of TEMPLATE_FOLDERS) {
    ...
    results.push({ templateId: folder, textCount, imageCount, outPath });
    log(`[placements] ${folder}: ${textCount} text + ${imageCount} image placements -> ${path.relative(process.cwd(), outPath)}`);
  }
